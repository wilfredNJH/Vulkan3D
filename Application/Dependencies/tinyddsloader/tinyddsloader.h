...
Result DDSFile::Load(std::vector<uint8_t>&& dds) {
    ...
    if (dxt10Header) {
        auto dxt10HeaderInner = reinterpret_cast<const HeaderDXT10*>(
            reinterpret_cast<const char*>(header) + sizeof(Header));

        m_arraySize = dxt10HeaderInner->m_arraySize;
        if (m_arraySize == 0) {
            return Result::ErrorInvalidData;
        }

        switch (dxt10HeaderInner->m_format) {
            case DXGIFormat::AI44:
            case DXGIFormat::IA44:
            case DXGIFormat::P8:
            case DXGIFormat::A8P8:
            ...
            default:
                if (GetBitsPerPixel(dxt10HeaderInner->m_format) == 0) {
                    return Result::ErrorNotSupported;
                }
        }

        m_format = dxt10HeaderInner->m_format;

        switch (dxt10HeaderInner->m_resourceDimension) {
            ...
            case TextureDimension::Texture2D:
                if (dxt10HeaderInner->m_miscFlag &
        ...
        m_texDim = dxt10HeaderInner->m_resourceDimension;
    } else {
